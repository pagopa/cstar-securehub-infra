...
resource "argocd_application" "domain_argocd_applications" {
  ...
  spec {
    ...
    source {
      repo_url        = "https://github.com/pagopa/mdc-aks-deploy"
      ...
      helm {
        values = yamlencode({
          microservice-chart : {
            azure : {
              workloadIdentityClientId : module.workload_identity.user_assigned_identity_client_id
            }
            serviceAccount : {
              name : module.workload_identity.user_assigned_identity_name
            }
          }
        })
