...
resource "null_resource" "trigger_create_tables_srtp_rtps_witchid" {
  triggers = {
    file_hash = sha256(templatefile("${path.module}/data_explorer_kql/create_tables_srtp_rtps_witchid.kql.tftpl", {
      soft_delete_period        = "${coalesce(var.adx_table_soft_delete_period_days, var.adx_db_soft_delete_period_days)}.00:00:00"
      hot_cache_period_timespan = "${coalesce(var.adx_table_hot_cache_period_days, var.adx_db_hot_cache_period_days)}.00:00:00"
      hot_cache_period          = "${coalesce(var.adx_table_hot_cache_period_days, var.adx_db_hot_cache_period_days)}d"
    }))
...
resource "azapi_resource" "create_tables_srtp_rtps_witchid" {
  type      = "Microsoft.Kusto/clusters/databases/scripts@2023-08-15"
  name      = "create-table-srtp-rtps-witchid"
  parent_id = "${data.azurerm_kusto_cluster.kusto_cluster.id}/databases/${var.domain}"

  body = {
    properties = {
      scriptContent = templatefile("${path.module}/data_explorer_kql/create_tables_srtp_rtps_witchid.kql.tftpl", {
        soft_delete_period        = "${coalesce(var.adx_table_soft_delete_period_days, var.adx_db_soft_delete_period_days)}.00:00:00"
        hot_cache_period_timespan = "${coalesce(var.adx_table_hot_cache_period_days, var.adx_db_hot_cache_period_days)}.00:00:00"
        hot_cache_period          = "${coalesce(var.adx_table_hot_cache_period_days, var.adx_db_hot_cache_period_days)}d"
      })
  ...
  lifecycle {
    replace_triggered_by = [null_resource.trigger_create_tables_srtp_rtps_witchid]
  }
}

resource "azurerm_data_factory_linked_service_kusto" "kusto_srtp" {
  name                     = "${var.domain}-Kusto-${var.domain}-ls"
  data_factory_id          = data.azurerm_data_factory.data_factory.id
  kusto_endpoint           = data.azurerm_kusto_cluster.kusto_cluster.uri
  kusto_database_name      = azurerm_kusto_database.db[var.domain].name
