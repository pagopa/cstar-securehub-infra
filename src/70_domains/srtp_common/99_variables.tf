...
variable "cosmos_rtp_db_autoscale_max_throughput" {
  type        = number
  description = "Max throughput for rtp database (shared across collections)"
  default     = null
}
...
variable "aks_nodepool_blue" {
  ...
  description = "Parameters for blue node pool"
}
