locals {
  ...
  eventhub_02_name           = "${local.project}-evh-02-ns"
...
# 🅒 Event Hub Namespace for IDPay
#
module "eventhub_namespace_idpay_02" {
  source = "./.terraform/modules/__v4__/eventhub"

  name                     = local.eventhub_02_name
  location                 = var.location
  resource_group_name      = data.azurerm_resource_group.idpay_data_rg.name
  auto_inflate_enabled     = var.ehns_auto_inflate_enabled
  sku                      = var.ehns_sku_name
...
module "event_hub_idpay_02_configuration" {
  source = "./.terraform/modules/__v4__/eventhub_configuration"

  event_hub_namespace_name                = module.eventhub_namespace_idpay_02.name
  event_hub_namespace_resource_group_name = module.eventhub_namespace_idpay_02.resource_group_name

  eventhubs = local.eventhubs_idpay_02
...
#tfsec:ignore:AZU023
resource "azurerm_key_vault_secret" "event_hub_idpay_02_primary_connection_string" {
  for_each = module.event_hub_idpay_02_configuration.key_ids

  name         = format("evh-%s-%s-idpay-02", replace(each.key, ".", "-"), "jaas-config")
  value        = format(local.jaas_config_template_idpay, module.event_hub_idpay_02_configuration.keys[each.key].primary_connection_string)
  content_type = "text/plain"
