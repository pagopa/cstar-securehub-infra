locals {
  ...
  spa_users = [
    ...
    {
      name  = replace(replace("SPA-${spa_users}", "-", ""), "/", "0")
      ...
      url_rewrite_action = {
        source_pattern          = "/${spa_users}/"
        destination             = "/${spa_users}/index.html"
...
module "cdn_idpay_usersportal" {
  ...
  hostname              = "portaleutenti.${data.azurerm_dns_zone.public_cstar.name}"
  https_rewrite_enabled = true

  storage_account_name             = "${local.project}userscdnsa"
  ...
  delivery_rule = [
    {
      name  = "robotsNoIndex"
      order = 3 + length(local.spa_users)
    ...
    },
    {
      ...
      order = 4 + length(local.spa_users)
