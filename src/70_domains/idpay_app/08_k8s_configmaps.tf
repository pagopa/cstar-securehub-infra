...
resource "kubernetes_config_map" "idpay-eventhub-00" {
  ...
  data = {
    kafka_broker                                       = local.eventhub_00_url
...
resource "kubernetes_config_map" "idpay-eventhub-01" {
  ...
  data = {
    kafka_broker            = local.eventhub_01_url
    kafka_sasl_mechanism    = "PLAIN"
    kafka_security_protocol = "SASL_SSL"
...
resource "kubernetes_config_map" "rest-client" {
  ...
  data = {
    ...
    idpay_timeline_host              = "http://idpay-timeline-microservice-chart:8080"
  }
...
resource "kubernetes_config_map" "appinsights-config" {
  ...
  data = {
    "applicationinsights.json" = file("./configs/k8s-file/appinsights-config/applicationinsights.json")
  }
}
