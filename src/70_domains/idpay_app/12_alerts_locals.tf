...
locals {
  ...
  | where ResultCode in ("400", "401", "429")
    ...
    pos_in_progress_transactions_list = {
      label = "POS In Progress Transactions List"
      alerts = {
  ...
  | where ResultCode in ("401", "404", "429")
    ...
    pos_processed_transactions_list = {
      label = "POS Processed Transactions List"
      alerts = {
  ...
  | where ResultCode startswith "5"
    ...
    retrieve_a_point_of_sale = {
      label = "Retrieve a point of sale"
      alerts = {
  ...
  | where ResultCode in ("401", "404", "429")
    ...
    get_initiative_id_onboarding_service = {
      label = "Get Initiative ID (Onboarding Service)"
      alerts = {
  ...
  | where ResultCode in ("401", "429")
    ...
    get_initiative_detail_onboarding_service = {
      label = "Get Initiative Detail (Onboarding Service)"
      alerts = {
  ...
  | where ResultCode in ("401", "429")
    ...
    onboarding_initiative_user_status = {
      label = "Onboarding Initiative User Status"
      alerts = {
  ...
  | where ResultCode startswith "5"
    ...
    get_initiative_beneficiary_detail = {
      label = "Get Initiative Beneficiary Detail"
      alerts = {
  ...
  | where ResultCode in ("401", "429")
    ...
    create_barcode_transaction = {
      label = "Create Barcode Transaction"
      alerts = {
  ...
  | where ResultCode in ("401", "429")
    ...
    retrieve_active_barcode_transaction = {
      label = "Retrieve Active Barcode Transaction"
      alerts = {
  ...
  alert_definitions = merge([
    for group in values(local.alert_groups) : group.alerts
  ]...)
}
