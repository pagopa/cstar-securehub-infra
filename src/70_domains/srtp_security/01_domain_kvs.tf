...
module "srtp_admins_policy" {
  ...
  key_vault_id      = module.key_vault[each.key].id
  tenant_id         = data.azurerm_client_config.current.tenant_id
  object_id         = data.azuread_group.srtp_adgroup_admin.object_id
}
...
module "srtp_developers_policy" {
  ...
  key_vault_id      = module.key_vault[each.key].id
  tenant_id         = data.azurerm_client_config.current.tenant_id
  object_id         = data.azuread_group.srtp_adgroup_developers.object_id
}
...
module "srtp_externals_policy" {
  ...
  key_vault_id      = module.key_vault[each.key].id
  tenant_id         = data.azurerm_client_config.current.tenant_id
  object_id         = data.azuread_group.srtp_adgroup_externals.object_id
}
