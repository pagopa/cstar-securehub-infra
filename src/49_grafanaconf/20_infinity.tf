...
resource "azuread_service_principal" "grafana_infinity" {
  client_id = azuread_application.grafana_infinity.client_id
}

resource "azuread_application_password" "grafana_infinity" {
  application_id = azuread_application.grafana_infinity.id
...
resource "grafana_data_source" "infinity" {
  ...
  json_data_encoded = jsonencode({
    ...
    oauth2 = {
      ...
      client_id     = azuread_service_principal.grafana_infinity.client_id
      scopes = [
        ""
      ],
...
resource "grafana_folder" "costs" {
  provider = grafana.cloud
  title    = "${upper(var.prefix)}-COSTS"
}

resource "grafana_dashboard" "costs" {
  provider = grafana.cloud

  config_json = templatefile(
    "${path.module}/custom_dashboard/azurecostanalysisinfinity.json",
    {
      env          = var.env,
  ...
  folder    = grafana_folder.costs.uid
  overwrite = true
}
