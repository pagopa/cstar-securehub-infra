...
resource "grafana_rule_group" "idpay_app_alerts" {
  ...
  folder_uid       = grafana_folder.alert_folders[local.alert_folders.idpay_alert_folder].uid
...
resource "grafana_notification_policy" "idpay_app_alerts" {
  ...
  policy {
    ...
    matcher {
      ...
      match = "="
      value = grafana_folder.alert_folders[local.alert_folders.idpay_alert_folder].title
    }
