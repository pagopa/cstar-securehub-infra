...
locals {
  custom_dashboard_by_file = fileset("${path.module}/env/${var.location_short}-${var.env}/dashboard/", "**")
...
resource "grafana_dashboard" "azure_monitor_grafana" {
  ...
  config_json = file("${path.module}/env/${var.location_short}-${var.env}/dashboard/${local.dashboard_file_map[each.value].filename}")
