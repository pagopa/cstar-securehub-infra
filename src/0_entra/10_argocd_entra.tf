...
module "argocd_entra" {
  ...
  argocd_hostname                = "argocd.${local.internal_dns_zone_name}"
  entra_app_owners_object_ids    = data.azuread_users.argocd_application_owners.object_ids
  entra_sp_groups_with_user_role = concat(local.entra_groups_allowed, var.argocd_entra_groups_allowed_extra)
